...
from kb.keyboard import admin_kb, admin_kb_cancel, send_post_kb
...
async def remove_post(message: types.Message, state: FSMContext, session: AsyncSession, bot: Bot):
...
async def remove_post_button(message: types.Message, state: FSMContext, session: AsyncSession):
    try:
        ...
        data = await orm_delete_post(session, post_id)
        if data:
...
async def add_post(message: types.Message, session: AsyncSession,  bot: Bot):
    ...
    await message.answer(
        "Here is your database:", reply_markup=admin_kb.as_markup(
            resize_keyboard=True))
    async with aiosqlite.connect('data_dump.db') as conn:
        await insert_users_from_dict(conn, users_dict)
    await bot.send_document(message.chat.id, FSInputFile("data_dump.db"))
